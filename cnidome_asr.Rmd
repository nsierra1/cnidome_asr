...
```{r Tree Cleanup, echo=FALSE}
  ...
  # Save to nexus for BEAUti import    
    if(save=="yes") {writeNexus(tree,paste0(root,"/data/pruned_tree.nex"))}
  ...
  # Tree info:
    ...
    #Ok, names between tree and character data and depnames have been ironed out, now need to check gene file names
...
```{r}
  ...
  for (type in cnidocytes){
    # Make an empty df, populate col1 with all species on tree
    states <- setNames(data.frame(matrix(ncol=2, nrow=length(tree$tip.label))), c("Species", "State"))
    states[,1]<-tree$tip.label
    
    #Populate col2
    #if the species in data has 

    # record species with cnidocyte present as a vector
    species_with <- as.vector(unique(data[which(data$Cnidocyte == type),2]))
    # use the vector to populate the df with 1s
    for (i in 1:nrow(states)) {
      if (states[i,1] %in% species_with) {states[i,2] = 1} # if the species is in spec_w_cnidocyte, add a 1
      if (states[i,1] %notin% unique(data$Species)) {states[i,2]="?"} # if the species is not in the charac data, add a ?
      else {states[i,2] = 0}
    }
    # save a tab file for the data (per intructions on BEAUti)
    ntax=length(tree$tip.label)
    names(states)<-c("traits",type)
    filename = paste0("data/",date,"/M2_", type, ".tab")
    write.table(states[,1:2], file=filename, sep="\t", quote=FALSE, row.names = F)
  }; rm(i, species_with,type,filename,states)
...
```{r message=FALSE}
  ...
  for (u in genes){
      ...
      # Save as new fasta file
        if(save=="yes"){write.FASTA(fasta,paste0(root,"/data/alignments_18/",u,"-pruned.fasta"))}
  }

  
    # Save list of species retained in gene files to a txt
    if(save=="yes"){if (sort(unique(fasta_Sp))==sort(tree$tip.label)){
    ...
    if(save=="yes"){write.table(sort(unique(trim_SpH)),file=paste0(root,"/data/alignments_18/fasta_species-trimmed.txt"),quote=F,row.names=F,col.names=F)}
...
```{r}
  ...
  alarm(suppressMessages(for (u in genes){
    x<-paste0("iqtree -s ",root,"/data/alignments_18/",u,"-pruned.fasta -pre ",u,"_",gsub(paste0(format(Sys.Date(), "%Y"),"-"),"",Sys.Date()),"_",c," -st DNA -mset JC,TN93,HKY,GTR -nt AUTO -m TESTNEWONLY")
    system(x)
  })); rm(c)
...
```{r}
  ...
  x<-'/usr/local/Cellar/beast/1.10.4/libexec/bin/beauti'

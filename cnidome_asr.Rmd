...
```{r}
  ...
  # Read in character data from online source
    ...
    # Save to a file
      write.table(subset(raw_data, select=-c(X,Key)),file=paste0(root,"/data/character_data/data_table/raw_character_",gsub(paste0(format(Sys.Date(), "%Y"),"-"),"",Sys.Date()),".txt"),quote=F,sep="\t",row.names=F)
  ...
  # Data Cleanup
    ...
    # Save to a file
      write.table(data,file=paste0(root,"/data/character_data/data_table/character_",gsub(paste0(format(Sys.Date(), "%Y"),"-"),"",Sys.Date()),".txt"),quote=F,sep="\t",row.names=F)
  ...
  for (type in cnidocytes){
    ...
    filename = paste0(root,"/data/character_data/",type,"_data_",gsub(paste0(format(Sys.Date(), "%Y"),"-"),"",Sys.Date()),".tab")
    write.table(states, file=filename, sep="\t", quote=FALSE, row.names = F)
  }; rm(i, spp_with,type,filename,states)

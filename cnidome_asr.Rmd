...
```{r setup, echo=F}
  ...
  knitr::opts_knit$set(root.dir = "~/set/path") ## SET PATH TO PROJECT DIRECTORY ROOT
  
  # Set up project directory
  root="/Users/noemie/Desktop/projects/cnidome/data/" ## SET PATH TO PROJECT DIRECTORY ROOT
...
```{r Tree Cleanup, echo=FALSE}
  ...
  # Remove existing branch lengths
    tree$edge.length<-NULL # remove existing branch lengths  
  # Save a copy of this tree
    write.tree(tree,paste0(root,"/pruned_tree.tre"))
...
```{r}
  ...
  # Use fossil calibration data to create a starting tree with branch lengths that do not disagree with fossil data
    ...
    # Run starttree
    x<-paste0("starttree -tre ",root,"pruned_tree.tre -cal ",root,"fossil_calibrations/nodes_fossilcalib.txt -out ",root,"pruned_tree.phy -n ",length(tree$tip.label))
    suppressMessages(system(x))
    
    # Save to nexus for BEAUti import 
    calib_tree<-read.tree(file=paste0(root,"/pruned_tree.phy"))
    writeNexus(calib_tree,paste0(root,"/pruned_tree_input.nex"))

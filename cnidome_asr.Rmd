...
Removes tips from tree without sufficiently specific nomenclature. Saves to a nexus file for use with IQTree/BEAST.
...
rm(SpH,fasta,fasta_Sp,u, trim_SpH,genes)
...
```{r}
    ...
    if (length(grep(paste(dup,collapse="|"),rownames(as.matrix(read.FASTA(paste0(root,"/data/alignments_18/16S-pruned.fasta"),type="DNA"))))) > 1) 


    # If there are more than 2 in the 16S-pruned.fasta:
    if (length(grep("Acropora_humilis",rownames(as.matrix(read.FASTA(paste0(root,"/data/alignments_18/16S-pruned.fasta"),type="DNA"))))) > 1) {
      
      # Open the two sequences as strings in a df with the accession number header
        Ach<-as.matrix(grep("Acropora_humilis",rownames(as.matrix(read.FASTA(paste0(root,"/data/alignments_18/16S-trimmed-unique.fasta"),type="DNA"))), value=T),value=T)
        Ach<-cbind(Ach,as.matrix(scan(paste0(root,"/data/alignments_18/16S-pruned.fasta"),what=character(),sep="\t")[grep("Acropora_humilis",scan(paste0(root,"/data/alignments_18/16S-pruned.fasta"),what=character(),sep="\t")) + 1]))
      ...
      # Find which sequence has more gaps ("-") to trim
        ...
        if (str_count(Ach[1,2],pattern="-") > str_count(Ach[2,2],pattern="-")){ 
            ...
            c=grep("^Acropora_humilis",rownames(as.matrix(read.FASTA(paste0(root,"/data/alignments_18/16S-pruned.fasta"),type="DNA"))))[1]
            # Remove from fasta
            write.FASTA(as.matrix(read.FASTA(paste0(root,"/data/alignments_18/16S-pruned.fasta"),type="DNA"))[-c,], paste0(root,"/data/alignments_18/16S-pruned.fasta"))
          ...
          write.table(sort(unique(trim_SpH)),file=paste0(root,"/data/alignments_18/fasta_species-trimmed.txt"),quote=F,row.names=F,col.names=F)
            ...
            c=grep("^Acropora_humilis",rownames(as.matrix(read.FASTA(paste0(root,"/data/alignments_18/16S-pruned.fasta"),type="DNA"))))[2]
            write.FASTA(as.matrix(read.FASTA(paste0(root,"/data/alignments_18/16S-pruned.fasta"),type="DNA"))[-c,], paste0(root,"/data/alignments_18/16S-pruned.fasta"))
  ...
  rm(Ach,fasta,file,c,Sp,u,accession,fasta_Sp)
  
```

# Change Outdated Nomenclature
```{r}

 # Replace the species names with most recent nomenclature from WoRMS
    # Load in the deprecated_names document
    dep<-read.csv(text=(gsheet2text('https://docs.google.com/spreadsheets/d/1T3-qX9tAqZ2CCQocp8CEVprsjDayoYmffNf_8F3_pzc/edit#gid=0')))
    dep<-dep[,1:2]; dep<-dep[dep[,2]!="",]; dep<-dep[dep[,1]!="",]
    # Replace species on the tree
    for (i in 1:length(tree$tip.label)){
      if(tree$tip.label[i] %in% dep$fixed_tree){tree$tip.label[i]<-as.character(dep$accepted_name[match(tree$tip.label[i],dep$fixed_tree)])}
    }
